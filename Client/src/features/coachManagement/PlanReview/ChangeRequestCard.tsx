      ...
      <article className="rounded-2xl border border-orange-100 bg-orange-50 p-5">
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <p className="truncate text-sm font-extrabold text-slate-900">{request.planTitle}</p>
            <p className="mt-0.5 text-xs text-slate-500">
              Trainee: <span className="font-semibold">{request.traineeName}</span>
            </p>
            <p className="mt-0.5 text-xs text-slate-400">Submitted: {formattedDate}</p>
          </div>
          <span className="shrink-0 rounded-full bg-orange-100 px-2.5 py-0.5 text-xs font-bold text-orange-700">
            Pending
          </span>
        </div>

        {/* Message */}
        <blockquote className="mt-3 rounded-xl border-l-4 border-orange-300 bg-white px-3 py-2 text-sm italic text-slate-700">
          "{displayMessage}"
        </blockquote>
        {isLong && (
          <button
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          <button
            ...
            className="rounded-2xl border border-red-200 bg-white px-4 py-2 text-xs font-bold text-red-700 transition hover:bg-red-50"
          >
            Reject request
          </button>
        </div>
