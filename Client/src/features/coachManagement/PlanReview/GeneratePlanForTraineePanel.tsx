...
const inp =
  "mt-1 w-full rounded-2xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-purple-400 disabled:opacity-50 dark:border-[#3B344A] dark:bg-[#2A2436] dark:text-[#C9C4D6] dark:placeholder:text-[#9E97AF]";
...
export function GeneratePlanForTraineePanel({
  ...
  const selectedTrainee = trainees.find((t) => t.userId === selectedTraineeId) ?? null;
  const isBlocked = selectedTraineeId !== "" && blockedTraineeIds.has(selectedTraineeId);
  ...
  const isButtonDisabled = !selectedTraineeId || isGenerating || traineesLoading || isBlocked;
  ...
  return (
    <div className="w-full rounded-3xl border border-purple-200 bg-white p-6 shadow-sm dark:border-purple-800/40 dark:bg-[#211D2B]">
      {traineesError && (
        ...
        <div className="space-y-4">
          <div>
            <label
              ...
              className="block text-xs font-bold uppercase tracking-wide text-slate-600 dark:text-[#9E97AF]"
            >
              Trainee
            </label>
            <select
              ...
              onChange={(e) => { setSelectedTraineeId(e.target.value); setGenerationError(""); }}
            ...
            >
              ...
              {trainees.map((trainee) => {
                ...
                return (
                  <option key={trainee.userId} value={trainee.userId} disabled={blocked}>
            ...
            {!traineesLoading && !traineesError && trainees.length === 0 && (
              <p className="mt-2 text-sm text-slate-400 dark:text-[#9E97AF]">
          ...
          {selectedTrainee && (
            <div className="rounded-2xl border border-purple-100 bg-purple-50 px-4 py-3 dark:border-purple-800/40 dark:bg-purple-900/20">
              <p className="text-sm font-bold text-slate-900 dark:text-[#F8F7FB]">
                {selectedTrainee.firstName} {selectedTrainee.lastName}
              </p>
              <p className="mt-0.5 text-xs capitalize text-slate-500 dark:text-[#9E97AF]">
                {selectedTrainee.level} · {selectedTrainee.status.replace("_", " ")}
              </p>
              {selectedTrainee.goals.length > 0 && (
                <p className="mt-1.5 text-xs text-slate-600 dark:text-[#C9C4D6]">
          ...
          {isBlocked && (
            <p className="rounded-2xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-sm font-medium text-amber-700 dark:border-amber-800/40 dark:bg-amber-900/20 dark:text-amber-400">
