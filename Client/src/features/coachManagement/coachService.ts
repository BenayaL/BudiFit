...
import type { Coach, Trainee, CoachPlan, CoachDashboardData, TraineeDetailsData } from "./coach.models";
...
export const coachService = {
  ...
  approvePlan: (planId: string, token: string): Promise<void> =>
    httpClient.post<void, Record<string, never>>(ENDPOINTS.coach.approvePlan(planId), {}, token),

  rejectPlan: (planId: string, token: string): Promise<void> =>
    httpClient.post<void, Record<string, never>>(ENDPOINTS.coach.rejectPlan(planId), {}, token),
