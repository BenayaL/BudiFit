...
export function TodayChallengeHero({ dashboard, isCompleting, onComplete, onChangePage }: Props) {
  ...
  if (!hasActivePlan || !planDay) {
    return (
      <HeroCard stripeColor="bg-slate-200">
        <p className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-5">
          Today's Challenge
        </p>
        <div className="flex items-center gap-4 mb-5">
          <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-slate-100 text-slate-400 flex-shrink-0">
            <Dumbbell size={22} />
          </div>
          <div>
            <h2 className="text-2xl font-extrabold tracking-tight text-slate-700 leading-tight">
              No active plan
            </h2>
    ...
    return (
      <HeroCard stripeColor="bg-gradient-to-r from-blue-300 to-sky-300">
        ...
        <div className="flex items-center gap-4">
          <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-50 flex-shrink-0 text-2xl">
            😴
          </div>
          <div>
            ...
            <h2 className="text-2xl font-extrabold tracking-tight text-slate-800 leading-tight">
              Recovery Day
            </h2>
          </div>
        </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-4 mb-5">
        <div
          className={`flex h-14 w-14 items-center justify-center rounded-2xl text-2xl flex-shrink-0 ${
            isCompleted ? "bg-emerald-50" : "bg-violet-50"
          }`}
        >
          {isCompleted ? "✅" : "🏋️"}
        </div>
      ...
      {exerciseNames.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-5 p-3 bg-slate-50 rounded-xl">
          {exerciseNames.map((name) => (
            <span
              key={name}
              className="text-xs font-semibold text-slate-600 px-2.5 py-1 bg-white rounded-full border border-slate-200"
            >
              {name}
            </span>
          ))}
      ...
      <div className="flex flex-wrap gap-2.5">
        ...
        <button
          onClick={() => onChangePage("workout")}
          className="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-50 text-slate-700 text-sm font-semibold rounded-xl border border-slate-200 hover:bg-slate-100 transition-colors"
        >
