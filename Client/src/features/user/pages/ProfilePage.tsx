...
// SVG icon helper — keeps JSX clean without a separate icon library
...
export function ProfilePage() {
  // Dummy data — future: fetch from userService.getProfile()
  ...
  const earnedCount = user.achievements.filter((a: Achievement) => a.earned).length;
    ...
    <div className="max-w-5xl mx-auto px-6 py-8">
      ...
      <div className="relative overflow-hidden rounded-[24px] bg-slate-900 shadow-lg">
        ...
        <div className="px-8 pb-8 flex flex-col sm:flex-row sm:items-end justify-between gap-6 -mt-12 relative z-10">
          ...
          >
            <Icon name="settings" className="w-4 h-4 mr-2 inline" /> Edit
            Profile
          </AppButton>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
        ...
        <StatTile
          label="Time Moved"
          value={Math.round(user.totalMinutes / 60).toString()}
          sub="hours"
          icon="clock"
        ...
        <StatTile
          label="Badges"
          value={`${earnedCount}/${user.achievements.length}`}
          sub="earned"
          icon="trophy"
