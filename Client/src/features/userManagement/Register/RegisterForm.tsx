...
export function RegisterForm({
  ...
  const inputClass =
    "w-full rounded-2xl border border-slate-200 bg-white px-5 py-4 text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-purple-500 focus:ring-4 focus:ring-purple-500/15 dark:border-[#3B344A] dark:bg-[#2A2436]/60 dark:text-[#F8F7FB] dark:placeholder:text-[#9E97AF]";
  ...
  return (
    <div className="rounded-[2rem] bg-white/90 p-8 shadow-[0_24px_80px_rgba(15,23,42,0.12)] ring-1 ring-slate-200 backdrop-blur sm:p-10 dark:bg-[#211D2B]/90 dark:ring-[#3B344A]">
      <div className="mb-8 text-center">
        <h2 className="text-3xl font-extrabold tracking-tight dark:text-[#F8F7FB]">Create account</h2>
        <p className="mt-2 text-slate-500 dark:text-[#9E97AF]">Start your BudiFit journey today.</p>
      </div>

      <form onSubmit={onSubmit} className="space-y-5">
        <div>
          <p className="mb-2 text-sm font-bold text-slate-700 dark:text-[#C9C4D6]">I want to join as</p>
            ...
            {(["trainee", "coach"] as const).map((role) => (
              <button
                ...
                className={`rounded-2xl border px-4 py-4 text-left transition ${
                  form.role === role
                    ? "border-purple-500 bg-purple-50 text-purple-700 ring-4 ring-purple-500/10 dark:bg-purple-900/30 dark:text-purple-300 dark:border-purple-600"
                    : "border-slate-200 bg-white text-slate-600 hover:border-purple-200 dark:border-[#3B344A] dark:bg-[#2A2436]/60 dark:text-[#9E97AF] dark:hover:border-purple-700"
                }`}
              >
        ...
        <p className="text-xs font-medium text-slate-400 dark:text-[#9E97AF]">Password must be at least 6 characters.</p>
      ...
      <div className="mt-8 text-center text-sm text-slate-500 dark:text-[#9E97AF]">
        ...
        <button type="button" onClick={onGoToLogin} className="font-bold text-purple-600 transition hover:text-purple-700 dark:text-purple-400 dark:hover:text-purple-300">
          Log in
        </button>
      </div>
    </div>
