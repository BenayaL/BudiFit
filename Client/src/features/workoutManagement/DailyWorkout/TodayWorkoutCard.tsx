    ...
    <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-[#3B344A] dark:bg-[#211D2B]">
      <div className="mb-4 flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500 dark:text-[#9E97AF]">{formatDate(today.date)}</p>
        </div>

        {isDone && (
          <span className="flex items-center gap-1.5 rounded-full bg-green-100 px-3 py-1 text-xs font-bold text-green-700 dark:bg-emerald-900/20 dark:text-emerald-400">
      ...
      {isRestDay ? (
        <div className="flex flex-col items-center py-6 text-center">
          ...
          <h3 className="mt-3 text-xl font-extrabold text-slate-900 dark:text-[#F8F7FB]">
            Rest Day
          </h3>
        ...
        <>
          <h3 className="text-xl font-extrabold text-slate-900 dark:text-[#F8F7FB]">
            {planDay!.title}
          </h3>
          ...
          <div className="mt-2 flex flex-wrap gap-3">
            ...
            {planDay!.durationMinutes > 0 && (
              <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600 dark:bg-[#2A2436] dark:text-[#C9C4D6]">
                ~{planDay!.durationMinutes} min
              </span>
            )}
          </div>

          <p className="mt-3 text-sm text-slate-500 line-clamp-2 dark:text-[#9E97AF]">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <button
              ...
              className="rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 shadow-sm transition hover:bg-slate-50 dark:border-[#3B344A] dark:bg-[#2A2436] dark:text-[#C9C4D6] dark:hover:bg-[#3B344A]"
            >
              View Workout
            </button>
      ...
      <p className="mt-4 text-xs text-slate-400 dark:text-[#9E97AF]">{activePlan.title}</p>
    </div>
  );
}
